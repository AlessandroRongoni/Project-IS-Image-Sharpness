...
%         title(['R, $$\nu =$$ ' sprintf('%d',v) ', $$k =$$ ' sprintf('%d',k)],'interpreter','latex')
        i=i+1;
...
%% GRAFICO
figure; subplot (1,2,1); plot(matrice_w'), xlabel('Iterations','interpreter','latex'), ylabel('Entries of the weight vector $$w$$','interpreter','latex'); 
%title('Grafico convergenza della rete neurale');
subplot(1,2,2); plot(conv), xlabel('Iterations','interpreter','latex'), ylabel('Value of the learning function $$\varphi(w)$$','interpreter','latex'); 
